...
describe("NewProject", () => {
  ...
  it("defaults to TERNARY priority style", () => {
    renderNewProject()
    const btn = screen.getByRole("button", { name: /ternary/i })
  ...
  it("switches to MOSCOW when clicked", () => {
    ...
    expect(screen.getByRole("button", { name: /moscow/i }).className).toContain(
      "bg-primary"
    )
  })
  ...
  it("switches back to TERNARY from MOSCOW", () => {
    ...
    expect(
      screen.getByRole("button", { name: /ternary/i }).className
  ...
  it("sends POST to /projects/new with correct payload", async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText(/ir-board system/i),
      "My Project"
    )
    await userEvent.type(
      screen.getByPlaceholderText(/university of oviedo/i),
      "UNIOVI"
    )
    await userEvent.type(
  ...
  it("shows 403 error message when server returns forbidden", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/you must be an admin/i)
      ).toBeInTheDocument()
    })
  })
  ...
  it("shows generic error message on non-403 failure", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/an error occurred while creating the project/i)
      ).toBeInTheDocument()
    })
  })
