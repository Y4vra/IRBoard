...
export default function StakeholderEdit() {
  ...
  const fetchStakeholder = useCallback(
    () =>
      fetch(
        `${API_BASE_URL}/projects/${projectId}/stakeholders/${stakeholderId}`,
        { credentials: "include" }
      ).then((r) => {
        if (!r.ok) throw new Error("Failed to fetch stakeholder details");
        return r.json();
      }),
  ...
  } = useBackendResource<Stakeholder>({ fetcher: fetchStakeholder });
  ...
  useEffect(() => {
    ...
    requestLock();
  }, [stakeholder, projectId, stakeholderId, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/projects/${projectId}/stakeholders/${stakeholderId}/modify`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
      ...
      if (!response.ok) {
        if (response.status === 403)
          throw new Error(
            "You do not have permission to modify this stakeholder."
          );
        if (response.status === 409)
          throw new Error(
            "The stakeholder is being edited by another user."
          );
        throw new Error("An error occurred while saving the stakeholder.");
      }

      navigate(`/project/${projectId}/stakeholders/${stakeholderId}`);
    } catch (err) {
  ...
  if (stakeholderLoading) return <LoadingSpinner text="Loading stakeholder..." />;
  ...
  if (stakeholderError || !stakeholder)
    return (
      <div className="mx-auto max-w-md mt-10 p-6 bg-red-50 border border-red-100 rounded-xl text-center">
        ...
        <p className="text-red-600 font-semibold">Could not load stakeholder</p>
        <p className="text-red-500 text-sm mt-1">{stakeholderError}</p>
        ...
        <div className="bg-card border border-border rounded-3xl shadow-xl overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                rows={4}
                placeholder="Describe this stakeholder's role and interests..."
            ...
            <div className="pt-4 flex items-center gap-3">
              <Button
                ...
                onClick={() =>
                  navigate(
                    `/project/${projectId}/stakeholders/${stakeholderId}`
                  )
                }
